...
public class GUI {
	...
	public static void createMainMenu(ReportPlugin pl, Player p) {
		int reportCount = pl.getReportManager().getUnresolvedReports().size();
		int finalSize = 9;
		...
		int index = 2;
		for (Report report : pl.getReportManager().getUnresolvedReports()) {
			main.setItem(index, GUI.createItemWithName(Material.BEACON, report.getID() + " " + report.getReporter()));
			index++;
		}
	}
	...
	public static void createReportMenu(ReportPlugin pl, Player p, Report r) {
		...
		report.setItem(5, GUI.createItemWithNameAndLore(Material.BOOK, "Report Description", r.getReport()));

		int index = 7;
		for (String s : r.getComments()) {
			report.setItem(index, GUI.createItemWithNameAndLore(Material.PAPER, "Comment " + (index - 5), s));
			index++;
		}
	}
